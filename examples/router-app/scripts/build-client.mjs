 ...
 * Client Build Script
 *
 * Builds the client bundle using esbuild with Aurelia transform.
...
console.log('Step 3: Building client bundle...');
...
const result = await esbuild.build({
  ...
  minify: true,
  sourcemap: true,
  // Resolve Aurelia packages from node_modules
  mainFields: ['module', 'main'],
  conditions: ['development'], // Use development builds for now
  metafile: true,
});

// Report bundle size
const outputFile = Object.keys(result.metafile.outputs).find(f => f.endsWith('main.js'));
if (outputFile) {
  const size = result.metafile.outputs[outputFile].bytes;
  console.log(`  Built: dist/assets/main.js (${(size / 1024).toFixed(1)} KB)`);
}

// Step 4: Copy and update index.html
console.log('Step 4: Processing index.html...');
let indexHtml = readFileSync(resolve(projectRoot, 'index.html'), 'utf-8');
