...
- [ ] Change AST node `span` types from `TextSpan` to `SourceSpan` (or add `sourceSpan?: SourceSpan` if doing a transitional shape).
- [ ] Update `BadExpression` to require `SourceSpan`; add optional `origin?: Origin` if desired.
- [ ] Update type aliases (`IsPrimary`, `IsAssign`, etc.) to the new span type.
- [ ] Ensure `Interpolation`, `ForOfStatement`, template nodes all use the new span shape.

#### 2) Span/provenance utilities (already present)
- [ ] Reuse `SourceSpan`, `absoluteSpan`, `ensureSpanFile` from `packages/domain/src/compiler/model/span.ts` and `packages/domain/src/compiler/model/source.ts`.
- [ ] Reuse `provenanceFromSpan` from `packages/domain/src/compiler/model/origin.ts` for `BadExpression`.

#### 3) Parser API/context (`packages/domain/src/parsers/lsp-expression-parser.ts`)
- [ ] Extend `parse` signatures to accept an optional parse context `{ baseSpan?: SourceSpan; file?: SourceFileId; baseOffset?: number }`.
- [ ] Thread context through `CoreParser` so every `spanFromBounds` call can emit a `SourceSpan` when context is present (`absoluteSpan` or `toSourceSpan` with offset).
- [ ] When parsing interpolation/template literals, propagate the same base offset/file to inner parses so embedded expressions are correctly rebased.
- [ ] Export `rebaseExpressionSpans(ast, baseSpan: SourceSpan)` helper that walks the AST (reuse `offsetNodeSpans`) and converts to `SourceSpan` with file.
- [ ] Construct `BadExpression` with `provenanceFromSpan("parse", sourceSpan)` when context is present; otherwise keep the current behavior.
...
- [ ] `packages/domain/src/compiler/phases/10-lower/lower-shared.ts`: pass `SourceSpan` (from `spanFromOffsets`) into the parser so AST spans are source-aware at creation; remove any post-parse offset hacks.
...
- [ ] Update parser tests under `packages/domain/test/parsers/*.test.mjs` for any signature/shape changes and expected spans/files.
- [ ] Update bind/lower/typecheck tests if they assert span shapes.
- [ ] Consider adding a test that parses with a `baseSpan`/`file` and asserts `span.file` is set on nested nodes (including interpolation segments).
