...
export function resolveImport(
  ...
  try {
    ...
    debug.project('resolveImport.step3', {
      resolvedPath,
    ...
    debug.project('resolveImport.step3.result', {
      found: !!exportBinding,
    ...
    debug.project('resolveImport.step4', {
      definitionPath: exportBinding.definitionPath,
    ...
    if (!definitionScope) {
      ...
      // Try on-demand resolution if available
      debug.project('resolveImport.step4.failed', { definitionPath: exportBinding.definitionPath });
      if (ctx.onDemandResolve) {
        const onDemandResult = tryOnDemandResolve(imp, ctx, fromFile);
        if (onDemandResult) {
          return onDemandResult;
    ...
    debug.project('resolveImport.step5', {
      definitionName: exportBinding.definitionName,
    ...
    if (!definition) {
      ...
      debug.project('resolveImport.step5.failed', { definitionName: exportBinding.definitionName });
      const reason = gap(
        `import "${imp.exportName}" from "${imp.specifier}"`,
...
function tryOnDemandResolve(
  ...
  if (ctx.resolving.has(cycleKey)) {
    debug.project('tryOnDemandResolve.cycle', { specifier: imp.specifier, exportName: imp.exportName });
    return null;
  }
  ctx.resolving.add(cycleKey);

  try {
    debug.project('tryOnDemandResolve.calling', {
    ...
    debug.project('tryOnDemandResolve.result', {
      specifier: imp.specifier,
      exportName: imp.exportName,
...
function lookupImportPath(
  ...
  if (!facts) {
    debug.project('lookupImportPath.noFileFacts', { fromFile });
    return null;
  }

  debug.project('lookupImportPath.fileFacts', {
    fromFile,
  ...
  for (const imp of facts.imports) {
    ...
    if (!imp.resolvedPath) {
      debug.project('lookupImportPath.noResolvedPath', {
        specifier,
    ...
    if (imp.kind === 'namespace') {
      ...
      debug.project('lookupImportPath.found', { specifier, kind: 'namespace', resolvedPath: imp.resolvedPath });
      return imp.resolvedPath;
    }
    if (imp.kind === 'default' && exportName === 'default') {
      debug.project('lookupImportPath.found', { specifier, kind: 'default', resolvedPath: imp.resolvedPath });
      return imp.resolvedPath;
    }
    if (imp.kind === 'named') {
      ...
      for (const binding of imp.bindings) {
        if (binding.name === exportName || binding.alias === exportName) {
          debug.project('lookupImportPath.found', { specifier, kind: 'named', exportName, resolvedPath: imp.resolvedPath });
          return imp.resolvedPath;
        }
      }
    }
  ...
  debug.project('lookupImportPath.notFound', { specifier, exportName });
  return null;
}
