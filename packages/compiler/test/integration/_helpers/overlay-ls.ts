...
interface LsSession {
  ...
  ensureFile: (path: string, text: string) => void;
  ensurePrelude: (path: string, text: string) => void;
  getSemanticDiagnostics: (path: string) => ts.Diagnostic[];
  cleanup: () => void;
  dispose: () => void;
}

function createLsSession({ checkJs }: { checkJs: boolean }): LsSession {
  const files = new Map<string, { text: string; version: number }>();
  const get = (f: string) => files.get(f);
  const put = (f: string, text: string) =>
    files.set(f, { text, version: (get(f)?.version ?? 0) + 1 });
  ...
  const ensureOnce = (path: string, text: string) => {
    if (!has(path)) put(path, text);
  };
  ...
  return {
    ...
    ensurePrelude(path, text) {
      ensureOnce(path, text);
    },
    getSemanticDiagnostics(path) {
      return service.getSemanticDiagnostics(path);
    },
    cleanup() {
...
let TS_SESSION: LsSession | undefined; // checkJs: false
let JS_SESSION: LsSession | undefined; // checkJs: true

export async function getSession(isJs: boolean): Promise<LsSession> {
  if (isJs) {
...
export interface CompileFromEntryOptions {
  html: string;
  markupFile?: string;
  isJs: boolean;
  className?: string;
...
export async function compileFromEntry({
  ...
  const overlay = new DefaultTemplateBuildService(program).getOverlay(uri)
    .overlay.text;
...
export function assertSuccess(
  ...
  if (diags.length) {
    const formatted = tsModule.formatDiagnosticsWithColorAndContext(diags, {
      getCurrentDirectory: () => "/mem",
      getCanonicalFileName: (f) => f,
      getNewLine: () => "\n",
    });
