...
export function getTestAppPath(appName: string, importMetaUrl: string): string {
  ...
  // Current layout: test/20-resolve/{category}/*.test.ts
  // Apps live at: test/20-resolve/apps/{appName}
  //
  // Fallback: if 20-resolve isn't present (older layout), resolve under test/apps.

  let testRoot = callerDir;
  while (!testRoot.endsWith("20-resolve") && path.dirname(testRoot) !== testRoot) {
    testRoot = path.dirname(testRoot);
  }

  if (testRoot.endsWith("20-resolve")) {
    return path.resolve(testRoot, "apps", appName);
  }

  // Legacy fallback: find the test root by looking for "test"
  testRoot = callerDir;
