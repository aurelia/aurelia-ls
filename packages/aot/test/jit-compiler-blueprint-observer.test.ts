...
describe("JIT compiler transformed-blueprint observer", () => {
  it("observes the complete registry as exact recursive canonical data", async () => {
    ...
    try {
      ...
      expect(JIT_ORACLE_CASES).toHaveLength(46);
      expect(first.observations).toHaveLength(46);
      expect(first.data.selectedCaseCount).toBe(46);
      expect(new Set(first.observations.map((observation) => observation.data.caseId)).size).toBe(46);
      ...
      expect(compiled.flatMap(({ outcome }) => outcome.definitions)).toHaveLength(70);
