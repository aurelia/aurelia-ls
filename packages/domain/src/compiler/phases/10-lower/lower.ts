...
function toNs(_el: P5Element): "html" | "svg" | "mathml" {
  // parse5 default adapter doesn't surface namespaces in this API; start with 'html'.
    ...
    if (isText(n)) {
      ...
      if (inter) {
        ...
        rows.push({
          target,
          instructions: [
            { type: "textBinding", from, loc: toSpan(n.sourceCodeLocation, table.file) },
          ],
        });
      }
      continue;
