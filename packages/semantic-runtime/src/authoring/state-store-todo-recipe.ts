...
import {
  stateStoreTodoSourcePlan,
  type StateStoreTodoSourcePlanModel,
} from './state-store-todo-source-plan.js';

export interface StateStoreTodoRecipeRequest {
...
type StateStoreTodoRecipeModel = StateStoreTodoSourcePlanModel;

export function buildStateStoreTodoPlan(request: StateStoreTodoRecipeRequest): AuthoringPlan {
  const model = normalizeStateStoreTodoRecipe(request);
  const topology = stateStoreTodoTopology(model);
  ...
  return new AuthoringPlan(
    new AuthoringIntent(
      ...
      topology,
      null,
      stateStoreTodoPreferences(),
    ),
    stateStoreTodoPreconditions(),
    stateStoreTodoPlanSteps(model, topology),
    topology,
    stateStoreTodoSourcePlan(model),
  );
}

function normalizeStateStoreTodoRecipe(request: StateStoreTodoRecipeRequest): StateStoreTodoRecipeModel {
  return {
    ...
    statePath: request.statePath ?? 'src/state/todo-store.ts',
  };
}

function stateStoreTodoPreferences(): readonly AuthoringPreference[] {
  return [
...
function stateStoreTodoPreconditions(): readonly AuthoringPrecondition[] {
  return [
...
function stateStoreTodoPlanSteps(
  model: StateStoreTodoRecipeModel,
  ...
  return [
    ...
    new AuthoringPlanStep(
      new ConfigurePluginOperation('StateDefaultConfiguration', '@aurelia/state'),
      [
    ...
    templateBindingPlanStep(
      ...
      stateStoreTemplateBindingExpectedEffects(),
    ),
    verifyAppPlanStep(topology, stateStoreTodoExpectedEffects()),
  ];
}

function stateStoreTodoTopology(model: StateStoreTodoRecipeModel): ApplicationTopology {
  ...
  builder.entrypoint({
    ...
    startupLane: 'new Aurelia().register(StandardConfiguration, StateDefaultConfiguration.init(...).withStore(...)).app(...).start()',
...
function stateStoreTodoExpectedEffects(): readonly ExpectedSemanticEffect[] {
  const prefix = 'Generated state-store todo app';
  return [
...
function stateStoreTemplateBindingExpectedEffects(): readonly ExpectedSemanticEffect[] {
  return [
