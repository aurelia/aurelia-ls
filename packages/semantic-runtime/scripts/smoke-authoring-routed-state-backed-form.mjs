...
import {
  ...
  buildRoutedStateBackedFormPlan,
...
const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'aurelia-ls-routed-state-backed-form-'));

const plan = buildRoutedStateBackedFormPlan({
  rootDir: tempRoot,
  appName: 'Generated Routed State Backed Form',
  rootComponentClassName: 'RoutedFormApp',
  rootElementName: 'routed-form-app',
});
await writeAuthoringSourcePlan(plan.sourcePlan, 'Routed state-backed form');

const runtime = await createSemanticRuntime({
  workspaceRoot,
  storeKey: 'generated-routed-state-backed-form-smoke',
  projects: [{
    rootDir: tempRoot,
