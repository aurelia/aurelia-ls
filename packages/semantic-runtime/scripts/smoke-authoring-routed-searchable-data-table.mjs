...
import {
  ...
  buildRoutedSearchableDataTablePlan,
...
const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'aurelia-ls-routed-searchable-data-table-'));
const plan = buildRoutedSearchableDataTablePlan({
  rootDir: tempRoot,
  appName: 'Generated Routed Searchable Data Table',
});
await writeAuthoringSourcePlan(plan.sourcePlan, 'Routed searchable data table');

const runtime = await createSemanticRuntime({
  workspaceRoot,
  storeKey: 'generated-routed-searchable-data-table-smoke',
  projects: [{
    rootDir: tempRoot,
    projectKey: 'generated-routed-searchable-data-table',
  }],
});
