...
import {
  ...
  buildRoutedServiceBackedFormPlan,
...
const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'aurelia-ls-routed-service-backed-form-'));

const plan = buildRoutedServiceBackedFormPlan({
  rootDir: tempRoot,
  appName: 'Generated Routed Service Backed Form',
  rootComponentClassName: 'RoutedServiceFormApp',
  rootElementName: 'routed-service-form-app',
});
await writeAuthoringSourcePlan(plan.sourcePlan, 'Routed service-backed form');

const runtime = await createSemanticRuntime({
  workspaceRoot,
  storeKey: 'generated-routed-service-backed-form-smoke',
  projects: [{
    rootDir: tempRoot,
    projectKey: 'generated-routed-service-backed-form',
  }],
});
