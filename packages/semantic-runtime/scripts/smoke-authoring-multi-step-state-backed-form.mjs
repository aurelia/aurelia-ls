...
import {
  ...
  buildMultiStepStateBackedFormPlan,
...
const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'aurelia-ls-multi-step-state-backed-form-'));

const plan = buildMultiStepStateBackedFormPlan({
  rootDir: tempRoot,
  appName: 'Generated Multi Step State Backed Form',
});
await writeAuthoringSourcePlan(plan.sourcePlan, 'Multi-step state-backed form');

const runtime = await createSemanticRuntime({
  workspaceRoot,
  storeKey: 'generated-multi-step-state-backed-form-smoke',
  projects: [{
    rootDir: tempRoot,
    projectKey: 'generated-multi-step-state-backed-form',
  }],
});
