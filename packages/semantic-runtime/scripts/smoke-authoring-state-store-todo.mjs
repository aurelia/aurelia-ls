...
const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'aurelia-ls-state-store-todo-'));

const plan = buildStateStoreTodoPlan({
  rootDir: tempRoot,
  appName: 'Generated State Store Todo',
});
await writeAuthoringSourcePlan(plan.sourcePlan, 'State store todo');

const runtime = await createSemanticRuntime({
  workspaceRoot,
  storeKey: 'generated-state-store-todo-smoke',
  projects: [{
    rootDir: tempRoot,
    projectKey: 'generated-state-store-todo',
  }],
});
