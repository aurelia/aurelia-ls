...
const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'aurelia-ls-localized-state-backed-form-'));

const plan = buildLocalizedStateBackedFormPlan({
  rootDir: tempRoot,
  appName: 'Generated Localized State Backed Form',
});
await writeAuthoringSourcePlan(plan.sourcePlan, 'Localized state-backed form');

const runtime = await createSemanticRuntime({
  workspaceRoot,
  storeKey: 'generated-localized-state-backed-form-smoke',
  projects: [{
    rootDir: tempRoot,
    projectKey: 'generated-localized-state-backed-form',
  }],
});
