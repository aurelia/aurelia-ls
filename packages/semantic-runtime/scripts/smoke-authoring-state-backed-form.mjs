...
const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'aurelia-ls-state-backed-form-'));

const plan = buildStateBackedFormPlan({
  rootDir: tempRoot,
  appName: 'Generated State Backed Form',
});
await writeAuthoringSourcePlan(plan.sourcePlan, 'State-backed form');

const runtime = await createSemanticRuntime({
  workspaceRoot,
  storeKey: 'generated-state-backed-form-smoke',
  projects: [{
    rootDir: tempRoot,
    projectKey: 'generated-state-backed-form',
  }],
});
const app = await runtime.openApp({ projectKey: 'generated-state-backed-form' });
