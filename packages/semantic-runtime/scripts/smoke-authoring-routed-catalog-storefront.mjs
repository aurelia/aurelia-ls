...
import {
  ...
  buildRoutedCatalogStorefrontPlan,
...
const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'aurelia-ls-routed-catalog-storefront-'));
const plan = buildRoutedCatalogStorefrontPlan({
  rootDir: tempRoot,
  appName: 'Generated Routed Catalog Storefront',
});
await writeAuthoringSourcePlan(plan.sourcePlan, 'Routed catalog storefront');

const runtime = await createSemanticRuntime({
  workspaceRoot,
  storeKey: 'generated-routed-catalog-storefront-smoke',
  projects: [{
    rootDir: tempRoot,
    projectKey: 'generated-routed-catalog-storefront',
  }],
});
