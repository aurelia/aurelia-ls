...
const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'aurelia-ls-localized-validated-state-backed-form-'));

const plan = buildLocalizedValidatedStateBackedFormPlan({
  rootDir: tempRoot,
  appName: 'Generated Localized Validated State Backed Form',
});
await writeAuthoringSourcePlan(plan.sourcePlan, 'Localized validated state-backed form');

const runtime = await createSemanticRuntime({
  workspaceRoot,
  storeKey: 'generated-localized-validated-state-backed-form-smoke',
  projects: [{
    rootDir: tempRoot,
    projectKey: 'generated-localized-validated-state-backed-form',
  }],
});
