...
This tracked fixture lane is reserved for internal generated app-code contracts
produced from the legacy app-builder ontology, policy, and source-lowering
surfaces. It is regression and evidence-harvesting infrastructure, not the
current public MCP authoring path.

Each future fixture should preserve:

- the exact internal app-builder request inputs that produced the source;
...
`app-builder-request.json` stores the reviewable internal request with
`rootDir: "."`, interpreted relative to the fixture root by the materializer.
`app-builder-response.json` stores the compact internal answer surface and
...
stored SourcePlan answer names that target, while `generated-control-use` means
...
groups, including grouped radio/checkbox-list fieldsets. The internal
source-lowering contract also rejects stale `ControlAccessibility` and
`VisualClassHooks` field scopes at
