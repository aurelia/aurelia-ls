...
const defaultFixturePath = join(
  packageDir,
  'fixtures',
  'aurelia-registration-productions',
  'golden.json',
);
...
async function main() {
  ...
  let api;
  try {
    api = await import('../out/aurelia-registration-production-goldens.js');
  } catch (error) {
    console.error('Unable to load built source-analysis output for aurelia registration production goldens.');
  ...
  const suite = api.collectAureliaRegistrationProductionGoldens({ repoPath });
  mkdirSync(dirname(outPath), { recursive: true });
  writeFileSync(outPath, `${JSON.stringify(suite, null, 2)}\n`);

  console.log(`Wrote Aurelia registration production goldens to ${outPath}`);
  console.log(`Productions: ${suite.summary.productionCount}`);
}
