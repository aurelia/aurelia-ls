...
The first pressure comes from editor, MCP, and agent use: a caller often starts from a workspace, project, file,
...
Projection lanes are intentionally consumer-neutral. A transport or presentation adapter can map LSP, MCP, a visual
...
editor/MCP entry points above the compiler products without creating a second completion path.
