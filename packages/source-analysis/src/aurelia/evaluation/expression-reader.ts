...
export class StaticEvaluationExpressionReader {
  ...
  evaluateExpression(expression: ts.Expression): EvaluationRead<EvaluationValue> {
    const result = new StaticEvaluator().evaluateExpressionInEnvironment(expression, this.environment, this.moduleKey);
  ...
  readExpressionTarget(expression: ts.Expression): EvaluationTargetRead {
    ...
    if (value?.kind === EvaluationValueKind.Class) {
      return readClassTarget(value.declaration, result.openSeams);
    }
