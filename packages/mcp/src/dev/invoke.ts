...
function parseInvocation(args: readonly string[]): {
  ...
  for (let index = 0; index < rest.length; index += 1) {
    ...
    if (key === '--resourceIdentityKey') {
      input.resourceIdentityKey = requireValue(rest, index, key);
      index += 1;
      continue;
    }
    if (key === '--templateResourceScopeIdentityKey') {
      input.templateResourceScopeIdentityKey = requireValue(rest, index, key);
      index += 1;
      continue;
    }
    if (key === '--group') {
    ...
    if (key === '--applicationEntrypointPolicy') {
      input.applicationEntrypointPolicy = requireValue(rest, index, key);
      index += 1;
      continue;
    }
    ...
    if (key === '--analysisLimitationPageSize') {
      input.analysisLimitationPageSize = parseNonNegativeInteger(requireValue(rest, index, key), key);
      index += 1;
      continue;
    }
...
function usage(): string {
  return [
    ...
    'Use --input <json> or a positional JSON object for full adapter input, plus common flags such as --query, --patternId, --documentPath, --documentPathPrefix, --sectionAnchor, --maxChars, --projectKey, --view, --projectRootHint, --excludedWorkspaceRoot, --analysisDepth, --applicationEntrypointPolicy, --includeAuthoringTemplates [true|false], --includeKernelBreakdowns [true|false], --includeDetailDensity [true|false], --includeQueryClaimRows [true|false], --includeAppProfile [true|false], --includeAppQueryClaimProfiles [true|false], --typeSystemDependencyCacheClearPolicy, --group, --queryKind, --frameworkCapability, --resourceIdentityKey, --templateResourceScopeIdentityKey, --sourceFile, --sourceFilePath (repeat for project-configurations), --cursor file:line:character[:offset], --diagnosticProjection, --analysisLimitationPageSize, --openSeamPageSize, --openSeamKindKey, --openSeamReasonKind, --sourceRole, --continuationIntent, --appRetention, --pageSize/--page.size, --pageCursor/--page.cursor, --projectPageSize/--projectPage.size, --projectPageCursor/--projectPage.cursor, --rowPageSize, and --rowLimit.',
  ].join('\n');
}
  ...
  return error.issues
    .map((issue) => {
      ...
      if (issue.code === 'invalid_value') {
        ...
        if (path === 'applicationEntrypointPolicy') {
          return 'applicationEntrypointPolicy: unsupported policy; use require-single-graph or aggregate-independent-graphs.';
        }
        if (path === 'detail') {
