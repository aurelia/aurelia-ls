...
describe("owned-template Extension Host contract", () => {
  ...
  test("pins the style-interpolation journey to the product-support shard used by both Worker lanes", () => {
    ...
    const containmentTitle = "suppresses native template diagnostics only through the scoped opt-in";
    ...
    expect(containmentJourney).toContain('suppression.update("suppressNative", true, vscode.ConfigurationTarget.WorkspaceFolder)');
    expect(containmentJourney).toContain('suppression.update("suppressNative", false, vscode.ConfigurationTarget.WorkspaceFolder)');
    ...
    expect(containmentJourney).toContain("Aurelia diagnostics should remain active during native suppression");
    expect(containmentJourney).toContain("an Aurelia diagnostic publication must not resurrect suppressed native diagnostics");
    expect(containmentJourney).toContain('aureliaWorkspace, "src", "unrelated.html"');
    expect(containmentJourney).toContain("unowned HTML must remain native while its workspace folder enables suppression");
    expect(containmentJourney).toContain("unowned HTML must not receive Aurelia template diagnostics");
    expect(containmentJourney).toContain("the opted-in template after its external association is withdrawn");
    expect(containmentJourney).toContain("withdrawn ownership should restore native diagnostics");
    expect(containmentJourney).toContain("the opted-in template after its external association is restored");
    expect(containmentJourney).toContain("restored exact ownership should resume enabled native suppression");
    expect(containmentJourney).toContain("template: '<div>temporarily inline</div>'");
    expect(containmentJourney).toContain('restoreWorkspaceFolderSettings(settingsSnapshot, "suppression cleanup")');
    expect(containmentJourney).toContain("settings-byte cleanup must leave the effective suppression value current");
    ...
    expect(productSurface).toContain("function snapshotWorkspaceFolderSettings(workspaceRoot)");
    expect(productSurface).toContain("function restoreWorkspaceFolderSettings(snapshot, label)");
    expect(productSurface).toContain("if (!snapshot.directoryExisted && existsSync(snapshot.directoryPath))");
    expect(productSurface).toContain("readdirSync(snapshot.directoryPath)");
    expect(productSurface).toContain("rmdirSync(snapshot.directoryPath)");
    expect(productSurface).not.toContain("rmdirSync(snapshot.directoryPath, { recursive: true })");
