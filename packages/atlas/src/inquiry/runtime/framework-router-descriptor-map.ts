...
export const ROUTE_RECOGNIZER_MECHANIC_DESCRIPTORS = new Map<string, RouteRecognizerMechanicDescriptor>(
  [
    ...
    recognizerMechanic("class:RecognizedRoute", "model", "endpoint-materialization", "recognized-route", null, "RecognizedRoute", "RecognizedRoute decodes ordinary parameters while retaining encoded residue for child routing, trims residual path suffixes, and stores the recognized path."),
...
export const ROUTER_FLOW_DESCRIPTORS = new Map<string, RouterFlowDescriptor>(
  [
    ...
    flowDescriptor("ParsedUrl._create", 14, "url-parsing", "ParsedUrl._create", "splits", "path/query/fragment", "ParsedUrl._create splits a router URL into path, query params, and fragment pieces before route expression parsing."),
    flowDescriptor("function:stringify", 15, "url-parsing", "router URL stringify", "serializes", "ParsedUrl or path/query/fragment", "The shared router stringify helper serializes parsed URL pieces for path and hash-fragment router modes."),
    flowDescriptor("pathUrlParser.parse", 16, "url-parsing", "pathUrlParser.parse", "parses", "path URL", "pathUrlParser.parse delegates path-mode navigation input to ParsedUrl._create."),
    flowDescriptor("pathUrlParser.stringify", 17, "url-parsing", "pathUrlParser.stringify", "serializes", "path URL", "pathUrlParser.stringify serializes path-mode navigation output through the shared router stringify helper."),
    flowDescriptor("fragmentUrlParser.parse", 18, "url-parsing", "fragmentUrlParser.parse", "parses", "hash-fragment URL", "fragmentUrlParser.parse extracts the hash fragment as the router path, normalizes slash-only input to the empty root path, then delegates query and fragment processing to ParsedUrl._create."),
    flowDescriptor("fragmentUrlParser.stringify", 19, "url-parsing", "fragmentUrlParser.stringify", "serializes", "hash-fragment URL", "fragmentUrlParser.stringify emits rooted hash-router URLs before serializing path/query/fragment pieces."),
    ...
    flowDescriptor("RouteConfig._create", 22, "route-config-authoring", "RouteConfig._create", "normalizes", "IRouteConfig | IChildRouteConfig | path", "RouteConfig._create normalizes strings, route objects, static properties, child routes, and navigation strategy inputs into RouteConfig while preserving a component's static nav setting."),
    flowDescriptor("RouteConfig._applyChildRouteConfig", 23, "route-config-authoring", "RouteConfig._applyChildRouteConfig", "overlays", "parent child-route config", "Child route config overlays clone the component route config without mutating the original metadata, preserving explicit ids and rederiving convention ids from an overridden path."),
    flowDescriptor("RouteConfig._applyFromConfigurationHook", 24, "route-config-authoring", "RouteConfig._applyFromConfigurationHook", "applies", "IRouteViewModel.getRouteConfig", "Instance getRouteConfig hooks can override RouteConfig fields after component creation; explicit ids become authoritative while convention ids follow an overridden path."),
    flowDescriptor("Route.getConfig", 30, "route-config-resolution", "Route.getConfig", "reads-or-creates", "route metadata", "Route.getConfig reads or creates route metadata and finalizes a convention-derived id once custom-element path metadata is available."),
    ...
    flowDescriptor("RouteContext.createViewportInstructions", 89, "viewport-instruction", "RouteContext.createViewportInstructions", "creates", "ViewportInstructionTree", "RouteContext.createViewportInstructions removes the deployment base, consumes root and repeated parent context prefixes with root clamping, preserves one shared context for arrays, and creates an internal tree without mutating caller-owned instructions."),
    ...
    flowDescriptor("function:createAndAppendNodes", 101, "route-tree-compilation", "createAndAppendNodes", "compiles", "ViewportInstruction -> RouteNode", "createAndAppendNodes recognizes paths, tries route-id lookup before fallback, hands whole-path residue from an empty parent route to configured child routes, and appends configured nodes."),
    ...
    flowDescriptor("RouteNode.create", 105, "route-tree-compilation", "RouteNode.create", "materializes", "RouteNode", "RouteNode.create freezes recognized params/query/data, records unresolved residue for nested routing, and exposes the node title as writable loading-hook state."),
    ...
    flowDescriptor("RouteContext.getRouteParameters", 114, "route-config-context", "RouteContext.getRouteParameters", "aggregates", "route and query parameters", "RouteContext.getRouteParameters walks active RouteContext parents, merges route params and optional query params by strategy, and freezes the returned parameter object."),
    ...
    flowDescriptor("LoadCustomAttribute.valueChanged", 130, "router-resource", "load.valueChanged", "creates", "link ViewportInstructionTree", "LoadCustomAttribute.valueChanged resolves route, params, and context into instructions, then applies the deployment base to generated href state in both path and hash modes."),
