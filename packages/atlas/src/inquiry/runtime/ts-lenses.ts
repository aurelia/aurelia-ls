...
import { HandleKind, HandleNamespace, type InquiryHandle, type SourceHandle, type SymbolHandle } from "../handle.js";
...
export function answerTsSource(
  ...
  const maxTextChars = projection === "text"
  ...
  return createAnswer(inquiry, outcome, sourceSummary(projection, targets.length, slices.length), {
    value,
    basis: [sourceTextBasis(sourceProject), programBasis(sourceProject)],
    evidence: targets.slice(0, evidenceLimit(inquiry)).map((target) => evidenceForTarget(target, EvidenceKind.SourceSpan)),
...
export function answerTsStructure(
  ...
  if (projection === "module-graph") {
    ...
    const outcome = moduleGraph.totalEdges === 0 ? OutcomeKind.Miss : OutcomeKind.Hit;
    return createAnswer(inquiry, outcome, `Returned ${moduleGraph.edges.length} of ${moduleGraph.totalEdges} TypeScript module edge(s).`, {
      value: { moduleGraph },
      basis: [programBasis(sourceProject)],
      evidence: moduleGraph.edges.slice(0, evidenceLimit(inquiry)).map(evidenceForModuleEdge),
      page: {
        size: moduleGraph.limit,
        cursor: inquiry.page?.cursor,
        returned: moduleGraph.edges.length,
        total: moduleGraph.totalEdges,
        ...(moduleGraph.nextOffset === undefined ? {} : { nextCursor: String(moduleGraph.nextOffset) }),
      },
      continuations: moduleGraphContinuations(inquiry, selector, moduleGraph),
    });
  }
  if (projection === "document-symbols") {
    const documentSymbols = readDocumentSymbols(sourceProject, selector, options);
    const outcome = documentSymbols.totalSymbols === 0 ? OutcomeKind.Miss : OutcomeKind.Hit;
    return createAnswer(inquiry, outcome, `Returned ${documentSymbols.symbols.length} of ${documentSymbols.totalSymbols} TypeScript document-symbol row(s).`, {
      value: { documentSymbols },
      basis: [programBasis(sourceProject)],
      evidence: documentSymbols.symbols.slice(0, evidenceLimit(inquiry)).map(evidenceForDocumentSymbol),
      page: {
        size: documentSymbols.limit,
        cursor: inquiry.page?.cursor,
        returned: documentSymbols.symbols.length,
        total: documentSymbols.totalSymbols,
        ...(documentSymbols.nextOffset === undefined ? {} : { nextCursor: String(documentSymbols.nextOffset) }),
      },
      continuations: documentSymbolContinuations(inquiry, selector, documentSymbols),
    });
  }
  if (projection === "symbols") {
    ...
    const outcome = symbols.totalEntries === 0 ? OutcomeKind.Miss : OutcomeKind.Hit;
    return createAnswer(inquiry, outcome, `Returned ${symbols.entries.length} of ${symbols.totalEntries} TypeScript symbol row(s).`, {
      value: { symbols },
      basis: [programBasis(sourceProject)],
      evidence: symbols.entries.slice(0, evidenceLimit(inquiry)).map((entry) => evidenceForTarget(entry.target, EvidenceKind.Symbol)),
      page: {
        size: symbols.limit,
        cursor: inquiry.page?.cursor,
        returned: symbols.entries.length,
        total: symbols.totalEntries,
        ...(symbols.nextOffset === undefined ? {} : { nextCursor: String(symbols.nextOffset) }),
      },
  ...
  if (projection === "exports") {
    ...
    const outcome = exports.totalExports === 0 ? OutcomeKind.Miss : OutcomeKind.Hit;
    return createAnswer(inquiry, outcome, `Returned ${exports.exports.length} of ${exports.totalExports} TypeScript export row(s).`, {
      value: { exports },
      basis: [programBasis(sourceProject), checkerBasis(sourceProject)],
      evidence: exports.exports.slice(0, evidenceLimit(inquiry)).map(evidenceForExportSurfaceEntry),
      page: {
        size: exports.limit,
        cursor: inquiry.page?.cursor,
        returned: exports.exports.length,
        total: exports.totalExports,
        ...(exports.nextOffset === undefined ? {} : { nextCursor: String(exports.nextOffset) }),
      },
  ...
  const outcome = surface.rollup.totalEntries === 0 ? OutcomeKind.Miss : OutcomeKind.Hit;

  return createAnswer(inquiry, outcome, `Returned ${returned} of ${surface.rollup.totalEntries} TypeScript declaration row(s).`, {
    value,
    basis: [programBasis(sourceProject)],
    evidence: surface.entries.slice(0, evidenceLimit(inquiry)).map((entry) => evidenceForTarget(entry.target, EvidenceKind.Symbol)),
    page: {
      size: surface.limit,
      cursor: inquiry.page?.cursor,
      returned,
      total: surface.rollup.totalEntries,
      ...(surface.nextOffset === undefined ? {} : { nextCursor: String(surface.nextOffset) }),
    },
...
export function answerTsType(
  ...
  if (projection === "references") {
    ...
    const outcome = references.totalReferences === 0 ? OutcomeKind.Miss : OutcomeKind.Hit;
    return createAnswer(inquiry, outcome, `Returned ${references.references.length} of ${references.totalReferences} TypeScript reference row(s).`, {
      value: { references },
      basis: [checkerBasis(sourceProject)],
      evidence: references.references.slice(0, evidenceLimit(inquiry)).map(evidenceForReference),
      page: {
        size: references.limit,
        cursor: inquiry.page?.cursor,
        returned: references.references.length,
        total: references.totalReferences,
        ...(references.nextOffset === undefined ? {} : { nextCursor: String(references.nextOffset) }),
      },
  ...
  if (projection === "definitions") {
    ...
    const outcome = navigation.totalEntries === 0 ? OutcomeKind.Miss : OutcomeKind.Hit;
    return createAnswer(inquiry, outcome, `Returned ${navigation.entries.length} of ${navigation.totalEntries} TypeScript navigation row(s).`, {
      value: { navigation },
      basis: [checkerBasis(sourceProject)],
      evidence: navigation.entries.slice(0, evidenceLimit(inquiry)).map(evidenceForNavigation),
      page: {
        size: navigation.limit,
        cursor: inquiry.page?.cursor,
        returned: navigation.entries.length,
        total: navigation.totalEntries,
        ...(navigation.nextOffset === undefined ? {} : { nextCursor: String(navigation.nextOffset) }),
      },
  ...
  if (projection === "call-hierarchy") {
    ...
    const outcome = callHierarchy.items.length === 0 && callHierarchy.totalEdges === 0 ? OutcomeKind.Miss : OutcomeKind.Hit;
    return createAnswer(inquiry, outcome, `Returned ${callHierarchy.edges.length} of ${callHierarchy.totalEdges} TypeScript call-hierarchy edge(s) from ${callHierarchy.items.length} item(s).`, {
      value: { callHierarchy },
      basis: [checkerBasis(sourceProject)],
      evidence: callHierarchy.edges.slice(0, evidenceLimit(inquiry)).map(evidenceForCallHierarchyEdge),
      page: {
        size: callHierarchy.limit,
        cursor: inquiry.page?.cursor,
        returned: callHierarchy.edges.length,
        total: callHierarchy.totalEdges,
        ...(callHierarchy.nextOffset === undefined ? {} : { nextCursor: String(callHierarchy.nextOffset) }),
      },
      continuations: callHierarchyContinuations(inquiry, selector, callHierarchy),
    });
  }
  if (projection === "call-sites") {
    ...
    const outcome = callSites.totalCallSites === 0 ? OutcomeKind.Miss : OutcomeKind.Hit;
    return createAnswer(inquiry, outcome, `Returned ${callSites.callSites.length} of ${callSites.totalCallSites} TypeScript call-site row(s).`, {
      value: { callSites },
      basis: [checkerBasis(sourceProject), sourceTextBasis(sourceProject)],
      evidence: callSites.callSites.slice(0, evidenceLimit(inquiry)).map(evidenceForCallSite),
      page: {
        size: callSites.limit,
        cursor: inquiry.page?.cursor,
        returned: callSites.callSites.length,
        total: callSites.totalCallSites,
        ...(callSites.nextOffset === undefined ? {} : { nextCursor: String(callSites.nextOffset) }),
      },
  ...
  if (projection === "diagnostics") {
    ...
    const outcome = diagnostics.totalDiagnostics === 0 ? OutcomeKind.Miss : OutcomeKind.Hit;
    return createAnswer(inquiry, outcome, `Returned ${diagnostics.diagnostics.length} of ${diagnostics.totalDiagnostics} TypeScript diagnostic row(s).`, {
      value: { diagnostics },
      basis: [checkerBasis(sourceProject)],
      evidence: diagnostics.diagnostics.slice(0, evidenceLimit(inquiry)).map(evidenceForDiagnostic),
      page: {
        size: diagnostics.limit,
        cursor: inquiry.page?.cursor,
        returned: diagnostics.diagnostics.length,
        total: diagnostics.totalDiagnostics,
        ...(diagnostics.nextOffset === undefined ? {} : { nextCursor: String(diagnostics.nextOffset) }),
      },
  ...
  if (projection === "quick-info") {
    ...
    const outcome = quickInfo.totalEntries === 0 ? OutcomeKind.Miss : OutcomeKind.Hit;
    return createAnswer(inquiry, outcome, `Returned ${quickInfo.entries.length} of ${quickInfo.totalEntries} TypeScript quick-info row(s).`, {
      value: { quickInfo },
      basis: [checkerBasis(sourceProject)],
      evidence: quickInfo.entries.slice(0, evidenceLimit(inquiry)).map(evidenceForQuickInfo),
      page: {
        size: quickInfo.limit,
        cursor: inquiry.page?.cursor,
        returned: quickInfo.entries.length,
        total: quickInfo.totalEntries,
        ...(quickInfo.nextOffset === undefined ? {} : { nextCursor: String(quickInfo.nextOffset) }),
      },
  ...
  if (projection === "signature-help") {
    ...
    const outcome = signatureHelp.totalItems === 0 ? OutcomeKind.Miss : OutcomeKind.Hit;
    return createAnswer(inquiry, outcome, `Returned ${signatureHelp.items.length} of ${signatureHelp.totalItems} TypeScript signature-help item(s).`, {
      value: { signatureHelp },
      basis: [checkerBasis(sourceProject)],
      evidence: signatureHelp.items.slice(0, evidenceLimit(inquiry)).map(evidenceForSignatureHelp),
      page: {
        size: signatureHelp.limit,
        cursor: inquiry.page?.cursor,
        returned: signatureHelp.items.length,
        total: signatureHelp.totalItems,
        ...(signatureHelp.nextOffset === undefined ? {} : { nextCursor: String(signatureHelp.nextOffset) }),
      },
      continuations: signatureHelpContinuations(inquiry, selector, signatureHelp),
    });
  }
  if (projection === "highlights") {
    const highlights = readHighlights(sourceProject, selector, pageOptions);
    const outcome = highlights.totalHighlights === 0 ? OutcomeKind.Miss : OutcomeKind.Hit;
    return createAnswer(inquiry, outcome, `Returned ${highlights.highlights.length} of ${highlights.totalHighlights} TypeScript highlight row(s).`, {
      value: { highlights },
      basis: [checkerBasis(sourceProject)],
      evidence: highlights.highlights.slice(0, evidenceLimit(inquiry)).map(evidenceForHighlight),
      page: {
        size: highlights.limit,
        cursor: inquiry.page?.cursor,
        returned: highlights.highlights.length,
        total: highlights.totalHighlights,
        ...(highlights.nextOffset === undefined ? {} : { nextCursor: String(highlights.nextOffset) }),
      },
  ...
  if (projection === "rename") {
    ...
    return createAnswer(inquiry, outcome, rename.canRename
      ...
      value: { rename },
      basis: [checkerBasis(sourceProject)],
      evidence: rename.locations.slice(0, evidenceLimit(inquiry)).map(evidenceForRenameLocation),
      page: {
        size: rename.limit,
        cursor: inquiry.page?.cursor,
        returned: rename.locations.length,
        total: rename.totalLocations,
        ...(rename.nextOffset === undefined ? {} : { nextCursor: String(rename.nextOffset) }),
      },
  ...
  if (projection === "refactors") {
    ...
    const outcome = refactors.totalActions === 0 ? OutcomeKind.Miss : OutcomeKind.Hit;
    return createAnswer(inquiry, outcome, `Returned ${refactors.actions.length} of ${refactors.totalActions} TypeScript refactor action(s).`, {
      value: { refactors },
      basis: [checkerBasis(sourceProject)],
      evidence: refactors.actions.slice(0, evidenceLimit(inquiry)).map(evidenceForRefactorAction),
      page: {
        size: refactors.limit,
        cursor: inquiry.page?.cursor,
        returned: refactors.actions.length,
        total: refactors.totalActions,
        ...(refactors.nextOffset === undefined ? {} : { nextCursor: String(refactors.nextOffset) }),
      },
  ...
  if (projection === "code-fixes") {
    ...
    const outcome = codeFixes.totalActions === 0 ? OutcomeKind.Miss : OutcomeKind.Hit;
    return createAnswer(inquiry, outcome, `Returned ${codeFixes.actions.length} of ${codeFixes.totalActions} TypeScript code-fix action(s).`, {
      value: { codeFixes },
      basis: [checkerBasis(sourceProject)],
      evidence: codeFixes.actions.slice(0, evidenceLimit(inquiry)).map(evidenceForCodeFixAction),
      page: {
        size: codeFixes.limit,
        cursor: inquiry.page?.cursor,
        returned: codeFixes.actions.length,
        total: codeFixes.totalActions,
        ...(codeFixes.nextOffset === undefined ? {} : { nextCursor: String(codeFixes.nextOffset) }),
      },
  ...
  if (projection === "refactor-edits") {
    ...
    const outcome = refactorEdits.applicable ? OutcomeKind.Hit : OutcomeKind.Miss;
    return createAnswer(inquiry, outcome, refactorEdits.applicable
      ...
      value: { refactorEdits },
      basis: [checkerBasis(sourceProject)],
      evidence: refactorEdits.changes.slice(0, evidenceLimit(inquiry)).map(evidenceForFileEdits),
  ...
  if (projection === "organize-imports") {
    ...
    const outcome = organizeImports.totalFiles === 0 ? OutcomeKind.Miss : OutcomeKind.Hit;
    return createAnswer(inquiry, outcome, `Returned ${organizeImports.changes.length} of ${organizeImports.totalFiles} TypeScript organize-import file edit group(s).`, {
      value: { organizeImports },
      basis: [checkerBasis(sourceProject)],
      evidence: organizeImports.changes.slice(0, evidenceLimit(inquiry)).map(evidenceForFileEdits),
      page: {
        size: organizeImports.limit,
        cursor: inquiry.page?.cursor,
        returned: organizeImports.changes.length,
        total: organizeImports.totalFiles,
        ...(organizeImports.nextOffset === undefined ? {} : { nextCursor: String(organizeImports.nextOffset) }),
      },
      continuations: organizeImportContinuations(inquiry, selector, organizeImports),
    });
  }
  if (projection === "file-rename-edits") {
    ...
    const outcome = fileRenameEdits.applicable ? OutcomeKind.Hit : OutcomeKind.Miss;
    return createAnswer(inquiry, outcome, fileRenameEdits.applicable
      ...
      basis: [checkerBasis(sourceProject)],
      evidence: fileRenameEdits.changes.slice(0, evidenceLimit(inquiry)).map(evidenceForFileEdits),
  ...
  const outcome = typeFacts.facts.length === 0 ? OutcomeKind.Miss : OutcomeKind.Hit;

  return createAnswer(inquiry, outcome, `Returned ${typeFacts.facts.length} TypeChecker fact row(s).`, {
    value: { typeFacts },
    basis: [checkerBasis(sourceProject)],
    evidence: typeFacts.facts.slice(0, evidenceLimit(inquiry)).map((fact) => evidenceForTarget(fact.target, EvidenceKind.TypeFact)),
  ...
  switch (inquiry.locus.kind) {
    ...
    case LocusKind.GitTree:
      return {
        ...
        ...(typeof inquiry.subject === "string" ? { query: inquiry.subject } : {}),
      };
  }
}
  ...
  switch (scheme) {
    ...
    case SourceSelectorScheme.Directory:
      return {
        ...
        ...(typeof source.recursive === "boolean" ? { recursive: source.recursive } : {}),
      };
...
function selectorFromHandle(handle: InquiryHandle): SourceSelector {
  if (handle.namespace === HandleNamespace.Source && handle.kind === HandleKind.SourceFile && "filePath" in handle) {
    return { scheme: SourceSelectorScheme.File, filePath: String(handle.filePath) };
  }
  if ((handle.namespace === HandleNamespace.Symbol || handle.namespace === HandleNamespace.TypeScript) && "name" in handle) {
    return {
      scheme: SourceSelectorScheme.Declaration,
      name: String(handle.name),
      ...("filePath" in handle && typeof handle.filePath === "string" ? { filePath: handle.filePath } : {}),
    };
  }
...
function positionFromRecord(source: Record<string, unknown>): { readonly line: number; readonly character: number } {
...
function sourceSummary(projection: string, targets: number, slices: number): string {
  if (projection === "text") {
...
function sourceContinuations(inquiry: Inquiry, targets: readonly SourceTargetRow[]): readonly Continuation[] {
...
function apiSurfaceContinuations(inquiry: Inquiry, selector: SourceSelector, surface: TypeScriptApiSurface): readonly Continuation[] {
...
function moduleGraphContinuations(inquiry: Inquiry, selector: SourceSelector, moduleGraph: TypeScriptModuleGraph): readonly Continuation[] {
  const continuations: Continuation[] = [];
  if (moduleGraph.nextOffset !== undefined) {
    continuations.push({
      ...
      inquiry: {
        ...inquiry,
        page: { size: moduleGraph.limit, cursor: String(moduleGraph.nextOffset) },
      },
  ...
  continuations.push({
    ...
    route: projectionRoute("Declaration surface for the same TypeScript selector."),
  });
  for (const [index, edge] of moduleGraph.edges.slice(0, 3).entries()) {
    continuations.push({
      ...
      inquiry: {
        lens: LensId.TsSource,
        locus: { kind: LocusKind.SourceRange, range: sourceRangeForModuleEdge(edge) },
        projection: "text",
      },
      evidence: [evidenceForModuleEdge(edge)],
      route: sourceRoute("Source import/export edge behind a module-graph row."),
    });
  }
  return continuations;
}

function documentSymbolContinuations(inquiry: Inquiry, selector: SourceSelector, documentSymbols: TypeScriptDocumentSymbolsRead): readonly Continuation[] {
  const continuations: Continuation[] = [];
  if (documentSymbols.nextOffset !== undefined) {
    continuations.push({
      ...
      inquiry: {
        ...inquiry,
        page: { size: documentSymbols.limit, cursor: String(documentSymbols.nextOffset) },
      },
  ...
  continuations.push({
    ...
    route: projectionRoute("Declaration rows for the same TypeScript selector."),
  });
  for (const [index, symbol] of documentSymbols.symbols.slice(0, 3).entries()) {
    continuations.push({
      ...
      inquiry: {
        lens: LensId.TsSource,
        locus: { kind: LocusKind.SourceRange, range: sourceRangeForDocumentSymbol(symbol) },
        projection: "text",
        budget: inquiry.budget,
      },
...
function symbolContinuations(inquiry: Inquiry, selector: SourceSelector, symbols: TypeScriptSymbolIndex): readonly Continuation[] {
  const continuations: Continuation[] = [];
...
function exportSurfaceContinuations(inquiry: Inquiry, selector: SourceSelector, exports: TypeScriptExportSurfaceRead): readonly Continuation[] {
  const continuations: Continuation[] = [];
  ...
  continuations.push({
    ...
    route: projectionRoute("API declaration surface for the same TypeScript selector."),
  });
  for (const [index, entry] of exports.exports.slice(0, 3).entries()) {
    ...
    if (range !== null) {
      continuations.push({
        ...
        evidence: [evidence],
        route: sourceRoute("Source declaration behind a TypeScript export row."),
      });
      continuations.push({
...
function typeContinuations(inquiry: Inquiry, typeFacts: TypeScriptTypeFacts): readonly Continuation[] {
  const continuations: Continuation[] = [{
...
function referenceContinuations(inquiry: Inquiry, selector: SourceSelector, references: TypeScriptReferenceRead): readonly Continuation[] {
  const continuations: Continuation[] = [];
  ...
  for (const [index, reference] of references.references.slice(0, 3).entries()) {
    continuations.push({
      ...
      inquiry: {
        lens: LensId.TsSource,
        locus: { kind: LocusKind.SourceRange, range: sourceRangeForReference(reference) },
        projection: "text",
        budget: inquiry.budget,
      },
...
function navigationContinuations(inquiry: Inquiry, selector: SourceSelector, navigation: TypeScriptNavigationRead): readonly Continuation[] {
  const continuations = typeProjectionBaseContinuations(inquiry, selector, "ts.type:definitions", navigation.nextOffset, navigation.limit, "Continue the TypeScript navigation page.");
  for (const [index, entry] of navigation.entries.slice(0, 3).entries()) {
    continuations.push({
      ...
      inquiry: {
        lens: LensId.TsSource,
        locus: { kind: LocusKind.SourceRange, range: sourceRangeForNavigation(entry) },
        projection: "text",
        budget: inquiry.budget,
      },
...
function callHierarchyContinuations(inquiry: Inquiry, selector: SourceSelector, callHierarchy: TypeScriptCallHierarchyRead): readonly Continuation[] {
  const continuations = typeProjectionBaseContinuations(inquiry, selector, "ts.type:call-hierarchy", callHierarchy.nextOffset, callHierarchy.limit, "Continue the TypeScript call-hierarchy page.");
  for (const [index, edge] of callHierarchy.edges.slice(0, 3).entries()) {
    ...
    continuations.push({
      ...
      kind: ContinuationKind.SwitchProjection,
      priority: ContinuationPriority.Primary,
      rationale: "Inspect exact callee and argument facts for this call-hierarchy edge.",
      inquiry: {
        lens: LensId.TsType,
        locus: { kind: LocusKind.SourceRange, range },
      ...
      route: callSitesRoute("Exact call-site facts behind a call-hierarchy edge."),
    });
    continuations.push({
...
function callSiteContinuations(inquiry: Inquiry, selector: SourceSelector, callSites: TypeScriptCallSitesRead): readonly Continuation[] {
  const continuations = typeProjectionBaseContinuations(inquiry, selector, "ts.type:call-sites", callSites.nextOffset, callSites.limit, "Continue the TypeScript call-site page.");
  for (const [index, callSite] of callSites.callSites.slice(0, 3).entries()) {
    continuations.push({
      ...
      inquiry: {
        lens: LensId.TsSource,
        locus: { kind: LocusKind.SourceRange, range: sourceRangeForCallSite(callSite) },
        projection: "text",
        budget: inquiry.budget,
      },
...
function diagnosticContinuations(inquiry: Inquiry, selector: SourceSelector, diagnostics: TypeScriptDiagnosticsRead): readonly Continuation[] {
  const continuations = typeProjectionBaseContinuations(inquiry, selector, "ts.type:diagnostics", diagnostics.nextOffset, diagnostics.limit, "Continue the TypeScript diagnostic page.");
  for (const [index, diagnostic] of diagnostics.diagnostics.slice(0, 3).entries()) {
...
function quickInfoContinuations(inquiry: Inquiry, selector: SourceSelector, quickInfo: TypeScriptQuickInfoRead): readonly Continuation[] {
  const continuations = typeProjectionBaseContinuations(inquiry, selector, "ts.type:quick-info", quickInfo.nextOffset, quickInfo.limit, "Continue the TypeScript quick-info page.");
  for (const [index, entry] of quickInfo.entries.slice(0, 3).entries()) {
    continuations.push({
      ...
      inquiry: {
        lens: LensId.TsSource,
        locus: { kind: LocusKind.SourceRange, range: sourceRangeForQuickInfo(entry) },
        projection: "text",
        budget: inquiry.budget,
      },
...
function signatureHelpContinuations(inquiry: Inquiry, selector: SourceSelector, signatureHelp: TypeScriptSignatureHelpRead): readonly Continuation[] {
  return typeProjectionBaseContinuations(inquiry, selector, "ts.type:signature-help", signatureHelp.nextOffset, signatureHelp.limit, "Continue the TypeScript signature-help page.");
}

function highlightContinuations(inquiry: Inquiry, selector: SourceSelector, highlights: TypeScriptHighlightsRead): readonly Continuation[] {
  const continuations = typeProjectionBaseContinuations(inquiry, selector, "ts.type:highlights", highlights.nextOffset, highlights.limit, "Continue the TypeScript highlight page.");
  for (const [index, highlight] of highlights.highlights.slice(0, 3).entries()) {
    continuations.push({
      ...
      inquiry: {
        lens: LensId.TsSource,
        locus: { kind: LocusKind.SourceRange, range: sourceRangeForHighlight(highlight) },
        projection: "text",
        budget: inquiry.budget,
      },
...
function renameContinuations(inquiry: Inquiry, selector: SourceSelector, rename: TypeScriptRenameRead): readonly Continuation[] {
  const continuations = typeProjectionBaseContinuations(inquiry, selector, "ts.type:rename", rename.nextOffset, rename.limit, "Continue the TypeScript rename-location page.");
  for (const [index, location] of rename.locations.slice(0, 3).entries()) {
    continuations.push({
      ...
      inquiry: {
        lens: LensId.TsSource,
        locus: { kind: LocusKind.SourceRange, range: sourceRangeForRenameLocation(location) },
        projection: "text",
        budget: inquiry.budget,
      },
...
function refactorContinuations(inquiry: Inquiry, selector: SourceSelector, refactors: TypeScriptRefactorsRead): readonly Continuation[] {
  return typeProjectionBaseContinuations(inquiry, selector, "ts.type:refactors", refactors.nextOffset, refactors.limit, "Continue the TypeScript refactor action page.");
}

function codeFixContinuations(inquiry: Inquiry, selector: SourceSelector, codeFixes: TypeScriptCodeFixesRead): readonly Continuation[] {
  return typeProjectionBaseContinuations(inquiry, selector, "ts.type:code-fixes", codeFixes.nextOffset, codeFixes.limit, "Continue the TypeScript code-fix action page.");
}

function refactorEditContinuations(inquiry: Inquiry, selector: SourceSelector, refactorEdits: TypeScriptRefactorEditsRead): readonly Continuation[] {
  ...
  return [{
    ...
    kind: ContinuationKind.SwitchProjection,
    priority: ContinuationPriority.Secondary,
    rationale: "Inspect applicable TypeScript refactor actions for this selector.",
    inquiry: {
      lens: LensId.TsType,
      locus: inquiry.locus,
      subject: selector,
    ...
    route: projectionRoute("Applicable refactor actions for the same TypeScript selector."),
  }];
}

function organizeImportContinuations(inquiry: Inquiry, selector: SourceSelector, organizeImports: TypeScriptOrganizeImportsRead): readonly Continuation[] {
  return typeProjectionBaseContinuations(inquiry, selector, "ts.type:organize-imports", organizeImports.nextOffset, organizeImports.limit, "Continue the TypeScript organize-imports edit page.");
}

function fileRenameEditContinuations(inquiry: Inquiry, selector: SourceSelector, fileRenameEdits: TypeScriptFileRenameEditsRead): readonly Continuation[] {
...
function projectionRoute(summary: string): NavigationRouteClaim {
  return continuationRoute(NavigationPlane.Structure, NavigationRelation.ProjectionOf, [BasisKind.TypeScriptProgram], summary);
}
...
function referencesRoute(summary: string): NavigationRouteClaim {
  return continuationRoute(NavigationPlane.Flow, NavigationRelation.ReferencesOf, [BasisKind.TypeScriptChecker], summary);
}

function callSitesRoute(summary: string): NavigationRouteClaim {
  return continuationRoute(NavigationPlane.Flow, NavigationRelation.CallSitesOf, [BasisKind.SourceText, BasisKind.TypeScriptChecker], summary);
}

function editPlanRoute(summary: string): NavigationRouteClaim {
  return continuationRoute(NavigationPlane.Maintenance, NavigationRelation.EditPlanFor, [BasisKind.TypeScriptChecker], summary);
}
...
function sourceRangeForDocumentSymbol(symbol: TypeScriptDocumentSymbolEntry): SourceRange {
  return sourceRangeFromFileSpan(symbol.file.repoPath, symbol.nameSpan ?? symbol.span);
}

function sourceRangeForReference(reference: TypeScriptReferenceEntry): SourceRange {
  return {
...
function sourceRangeForNavigation(entry: TypeScriptNavigationEntry): SourceRange {
  return sourceRangeFromFileSpan(entry.file.repoPath, entry.span);
}

function sourceRangeForCallHierarchyEdge(edge: TypeScriptCallHierarchyEdge): SourceRange | null {
  const span = edge.fromSpans[0];
  return span === undefined ? null : sourceRangeFromFileSpan(edge.from.file.repoPath, span);
}

function sourceRangeForCallSite(callSite: TypeScriptCallSiteEntry): SourceRange {
  return sourceRangeFromFileSpan(callSite.file.repoPath, callSite.span);
}

function sourceRangeForDiagnostic(diagnostic: TypeScriptDiagnosticEntry): SourceRange | null {
...
function sourceRangeForHighlight(highlight: TypeScriptHighlightEntry): SourceRange {
  return sourceRangeFromFileSpan(highlight.file.repoPath, highlight.span);
}

function sourceRangeForRenameLocation(location: TypeScriptRenameLocationEntry): SourceRange {
  return sourceRangeFromFileSpan(location.file.repoPath, location.span);
}

function sourceRangeForFileEdits(edits: TypeScriptFileEdits): SourceRange | null {
  const first = edits.edits.find((edit) => edit.span !== undefined);
  return first?.span === undefined ? null : sourceRangeFromFileSpan(edits.file.repoPath, first.span);
}

function sourceRangeFromFileSpan(filePath: string, span: { readonly startLine: number; readonly startCharacter: number; readonly endLine: number; readonly endCharacter: number }): SourceRange {
  return {
    filePath,
    start: {
...
function evidenceForTarget(target: SourceTargetRow, kind: EvidenceKind): Evidence {
...
function evidenceForReference(reference: TypeScriptReferenceEntry): Evidence {
  ...
  const handle: SourceHandle = {
    ...
    summary: reference.definition ? "definition" : reference.writeAccess ? "write-reference" : "reference",
    filePath: reference.file.repoPath,
  };
  return {
    id: reference.id,
    ...
    confidence: EvidenceConfidence.Exact,
    summary: `${reference.definition ? "definition" : "reference"} ${reference.text}`,
    source,
    handle,
    data: reference,
  };
...
function evidenceForDocumentSymbol(symbol: TypeScriptDocumentSymbolEntry): Evidence {
...
function evidenceForExportSurfaceEntry(entry: TypeScriptExportSurfaceEntry): Evidence {
  const firstTarget = entry.targets[0];
  const source = firstTarget === undefined ? null : sourceRangeForTarget(firstTarget);
...
function evidenceForNavigation(entry: TypeScriptNavigationEntry): Evidence {
  ...
  return {
    ...
    role: EvidenceRole.Subject,
    confidence: EvidenceConfidence.Exact,
    summary: `${entry.kind} ${entry.name ?? entry.display ?? entry.file.repoPath}`,
    source,
    handle,
    data: entry,
  };
...
function evidenceForCallHierarchyEdge(edge: TypeScriptCallHierarchyEdge): Evidence {
...
function evidenceForDiagnostic(diagnostic: TypeScriptDiagnosticEntry): Evidence {
...
function evidenceForSignatureHelp(entry: TypeScriptSignatureHelpEntry): Evidence {
  ...
  const handle: SymbolHandle = {
    ...
    name: entry.display,
    ...(entry.target.file === undefined ? {} : { filePath: entry.target.file.repoPath }),
  };
  return {
    id: entry.id,
    kind: EvidenceKind.TypeFact,
...
function evidenceForHighlight(highlight: TypeScriptHighlightEntry): Evidence {
  ...
  return {
    id: highlight.id,
    kind: EvidenceKind.SourceSpan,
    role: highlight.kind === "definition" ? EvidenceRole.Subject : EvidenceRole.Support,
    confidence: EvidenceConfidence.Exact,
...
function evidenceForRenameLocation(location: TypeScriptRenameLocationEntry): Evidence {
...
function evidenceForRefactorAction(action: TypeScriptRefactorActionRow): Evidence {
  const handle: SymbolHandle = {
    namespace: HandleNamespace.TypeScript,
    kind: HandleKind.TypeFact,
...
function evidenceForCodeFixAction(action: TypeScriptCodeFixActionRow): Evidence {
  ...
  const handle: SymbolHandle = {
    ...
    name: action.fixName,
    ...(action.diagnostic.file === undefined ? {} : { filePath: action.diagnostic.file.repoPath }),
  };
  return {
    id: action.id,
    kind: EvidenceKind.TypeFact,
...
function handleForTarget(target: SourceTargetRow, kind: EvidenceKind): InquiryHandle {
  ...
  if (target.kind === SourceTargetKind.Declaration || target.kind === SourceTargetKind.Symbol || kind === EvidenceKind.Symbol) {
    const handle: SymbolHandle = {
      namespace: HandleNamespace.Symbol,
      kind: HandleKind.Symbol,
      id: target.id,
  ...
  const handle: SourceHandle = {
    namespace: HandleNamespace.Source,
    kind: target.kind === SourceTargetKind.SourceFile ? HandleKind.SourceFile : HandleKind.SourceRange,
    id: target.id,
    label: target.label,
    summary: target.declarationKind,
    filePath: target.file?.repoPath ?? "",
...
function stringFilter(filters: Record<string, unknown> | undefined, key: string): { readonly [key: string]: string } {
...
function sourceTextBasis(sourceProject: SourceProject): Basis {
  return {
    ...
    summary: "Answered from source files admitted into the hot Atlas SourceProject.",
...
function programBasis(sourceProject: SourceProject): Basis {
  return {
    ...
    freshness: BasisFreshness.Live,
    summary: "Answered from the current TypeScript Program held by the Atlas daemon.",
    identity: sourceProject.snapshot().identity,
  };
}
...
function checkerBasis(sourceProject: SourceProject): Basis {
  return {
    ...
    authority: BasisAuthority.Checker,
    freshness: BasisFreshness.Live,
    summary: "Answered from the current TypeScript TypeChecker held by the Atlas daemon.",
    identity: sourceProject.snapshot().identity,
  };
}
