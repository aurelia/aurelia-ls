...
`?aurelia-aot` modules. A complete convention view-definition artifact is the
sole compiler-final resource realization: its identity is recorded against the
path-addressed HTML module, and the owning source module does not claim a
duplicate compiler patch or apply runtime. The official conventions transform
attaches that namespace at the final class location. An exact template-value
